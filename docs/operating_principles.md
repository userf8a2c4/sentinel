...
- no favorece ni perjudica a partidos, candidatos o instituciones
- no emite juicios de valor
- no infiere intención humana
- no asigna responsabilidades

Todos los datos se tratan de forma idéntica,
independientemente de su impacto político.
...
- todas las fuentes de datos son públicas
- todo el código es abierto
- todas las reglas están documentadas
- todos los procesos son deterministas
- todos los resultados son verificables

Si un resultado no puede reproducirse, se considera inválido.
...
- versión del código utilizada

Los datos nunca se sobrescriben.
Los cambios solo se observan mediante comparación entre versiones.
...
- no existe selección editorial de eventos

Una publicación ocurre únicamente cuando
una regla documentada se activa.
...
Y emplea únicamente descriptores observacionales como:
- variación
- inconsistencia
- desviación
- evento atípico
...
El lenguaje es parte del diseño del sistema.
...
- no modifica reglas durante eventos en curso

Su credibilidad se basa en la constancia,
no en hallazgos aislados.
