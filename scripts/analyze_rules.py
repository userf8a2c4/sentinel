...
def run_audit(target_directory='data/normalized'):
    ...
    df["prev_total_votes"] = df.groupby("departamento")["total_votes"].shift(1)
    df["relative_delta_pct"] = df.apply(
        lambda row: (row["delta_votes"] / row["prev_total_votes"] * 100)
        if row["prev_total_votes"] else None,
        axis=1,
    )
    df["delta_escrutado"] = df.groupby("departamento")["porcentaje_escrutado"].diff()
    ...
    for departamento, group in df.groupby("departamento"):
        ...
        for _, row in group.iterrows():
            ...
            if row.get("relative_delta_pct") is not None and abs(row["relative_delta_pct"]) >= relative_threshold:
                anomalies.append({
                    "departamento": departamento,
                    "type": "RELATIVE_DELTA",
                    "timestamp": row["timestamp"].isoformat(),
                    ...
                    "file": row.get("file"),
                    "threshold_pct": relative_threshold,
                })
            if row.get("delta_escrutado") is not None and abs(row["delta_escrutado"]) >= scrutiny_jump_threshold:
                anomalies.append({
                    "departamento": departamento,
                    "type": "SCRUTINY_JUMP",
                    "timestamp": row["timestamp"].isoformat(),
                    "delta_escrutado": row["delta_escrutado"],
                    "porcentaje_escrutado": row.get("porcentaje_escrutado"),
                    "file": row.get("file"),
                    "threshold_pct": scrutiny_jump_threshold,
                })
