...
# Probabilidades esperadas para el segundo dígito (Benford 2BL).
# Expected probabilities for the second digit (Benford 2BL).
...
def extract_second_digit(number: int) -> int:
    ...
    Extract the second significant digit from an integer.
    ...
    str_num = str(number)
    if len(str_num) < 2:
        # Para números de 1 dígito, consideramos 0 como segundo.
        # For 1-digit numbers, we treat 0 as the second digit.
        return 0
...
def benford_second_digit_test(vote_counts: List[int]) -> Tuple[float, str]:
    ...
    # Conteo observado por dígito 0-9.
    # Observed count per digit 0-9.
    observed = [0] * 10
    valid_count = 0
    ...
    if valid_count < 50:
        # Muy pocos datos → no confiable.
        # Too few data points → not reliable.
